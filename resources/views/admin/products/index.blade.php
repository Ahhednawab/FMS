          ...
          <tbody>
            @foreach($products as $key => $value)
              <tr>
                ...
                <td>{{$value->quantity}} {{$value->product->unit}}</td>
                <td>{{$value->available}} {{$value->product->unit}}</td>
                <td>{{$value->restock_qty_alarm}} {{$value->product->unit}}</td>
                <td class="text-center">
                  <div class="list-icons">
                    <div class="dropdown">
                      <a href="#" class="list-icons-item" data-toggle="dropdown"><i class="icon-menu9"></i></a>
                      <div class="dropdown-menu dropdown-menu-right">
                        <a href="{{ route('admin.products.show', $value->id) }}" class="dropdown-item"><i class="icon-eye"></i> View</a>
                        <a href="{{ route('admin.products.edit', $value->id) }}" class="dropdown-item"><i class="icon-pencil7"></i> Edit</a>
                        <form action="{{ route('admin.products.destroy', $value->id) }}" method="POST" class="d-inline">
                          @csrf @method('DELETE')

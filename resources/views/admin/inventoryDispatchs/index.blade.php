...
@section('title', 'Inventory Dispatch List')
  ...
  <div class="page-header page-header-light">
    <div class="page-header-content header-elements-lg-inline">
      <div class="page-title d-flex">
        <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Inventory Dispatch List</span></h4>
        <a href="#" class="header-elements-toggle text-body d-lg-none"><i class="icon-more"></i></a>
      </div>
      <div class="header-elements d-none">
        <div class="d-flex justify-content-center">
          <a href="{{ route('admin.inventoryDispatchs.create') }}" class="btn btn-primary">
            <span>Add Inventory Dispatch <i class="icon-plus3 ml-2"></i></span>
          </a>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-body">
        <table class="table datatable-colvis-basic dataTable">
          ...
          <tbody>
            @foreach($inventoryDispatchs as $key => $value)
              <tr>
                <td>{{$value->serial_no}}</td>
                <td>{{ \Carbon\Carbon::parse($value->dispatch_date)->format('d-M-Y') }}</td>
                <td>{{$value->department->name}}</td>
                <td>{{$value->inventoryDispatchStatus->name}}</td>
                  ...
                  <div class="list-icons">
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        <a href="{{ route('admin.inventoryDispatchs.show', $value->id) }}" class="dropdown-item">
                          <i class="icon-eye"></i> View Details
                        </a>
                        <a href="{{ route('admin.inventoryDispatchs.edit', $value->id) }}" class="dropdown-item">
                          <i class="icon-pencil7"></i> Edit
                        </a>
                        <form action="{{ route('admin.inventoryDispatchs.destroy', $value->id) }}" method="POST">
                          @csrf
                          @method('DELETE')

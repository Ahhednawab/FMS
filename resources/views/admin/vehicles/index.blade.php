...
@section('content')
                    ...
                    <tbody>
                        @foreach ($vehicles as $key => $value)
                            <tr data-id="{{ $value->id }}">
                                ...
                                <td>{{ $value->insurance_policy_no }}</td>
    ...
    <script>
        ...
        $(document).ready(function() {
            const cookieName = 'vehicle_list_column_visibility';
            let savedVisibility = getCookie(cookieName);
            savedVisibility = savedVisibility ? JSON.parse(savedVisibility) : null;
            ...
            setTimeout(function() {
                ...
                try {
                    ...
                    console.error('Invalid visibility cookie');
                    return;
                }
            ...
            document.querySelectorAll('.dt-button.buttons-colvis').forEach(function(toggleBtn) {
                toggleBtn.addEventListener('click', function() {

                    // Delay slightly to allow dropdown items to render
                    setTimeout(function() {

                        // Select all buttons inside the dropdown
                        document.querySelectorAll('.dt-button-collection button').forEach(
                            function(btn) {

                                const colIdx = btn.getAttribute('data-cv-idx');

                                if (colIdx !== null && visibility[colIdx] === false) {
                                    // Remove active class for hidden columns
                                    btn.classList.remove('active');
                                    btn.setAttribute('aria-pressed', 'false');
                                }
                    ...
                    }, 50); // small timeout to ensure dropdown is added to DOM
                });
            });
            ...
            //                 if (savedVisibility.hasOwnProperty(index)) {
            //                     this.visible(savedVisibility[index]);
            //                 }
            //             });
            //         }
            //     }

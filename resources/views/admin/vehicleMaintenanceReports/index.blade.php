...
@section('title', 'Vehicle Maintenance Report List')
  ...
  <div class="page-header page-header-light">
    <div class="page-header-content header-elements-lg-inline">
      <div class="page-title d-flex">
        <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Vehicle Maintenance Report List</span></h4>
        <a href="#" class="header-elements-toggle text-body d-lg-none"><i class="icon-more"></i></a>
      </div>
      <div class="header-elements d-none">
        <div class="d-flex justify-content-center">
          <a href="{{ route('admin.vehicleMaintenanceReports.create') }}" class="btn btn-primary">
            <span>Add Vehicle Maintenance Report <i class="icon-plus3 ml-2"></i></span>
          </a>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-body">
        <table class="table datatable-colvis-basic dataTable">
          ...
          <tbody>
            @foreach($vehicleMaintenanceReports as $key => $value)
              <tr>
                <td>{{$value->maintenance_report_id}}</td>
                <td>{{$value->vehicle->vehicle_no}}</td>
                <td>{{$value->fuelType->name}}</td>
                <td>{{$value->maintenanceCategory->category}}</td>
                <td>{{$value->parts->name}}</td>
                  ...
                  <div class="list-icons">
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        <a href="{{ route('admin.vehicleMaintenanceReports.show', $value->id) }}" class="dropdown-item">
                          <i class="icon-eye"></i> View Details
                        </a>
                        <a href="{{ route('admin.vehicleMaintenanceReports.edit', $value->id) }}" class="dropdown-item">
                          <i class="icon-pencil7"></i> Edit
                        </a>
                        <form action="{{ route('admin.vehicleMaintenanceReports.destroy', $value->id) }}" method="POST">
                          @csrf
                          @method('DELETE')

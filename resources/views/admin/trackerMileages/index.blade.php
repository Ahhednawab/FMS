...
@section('title', 'Tracker Mileage List')
  ...
  <div class="page-header page-header-light">
    <div class="page-header-content header-elements-lg-inline">
      <div class="page-title d-flex">
        <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Tracker Mileage List</span></h4>
        <a href="#" class="header-elements-toggle text-body d-lg-none"><i class="icon-more"></i></a>
      </div>
      <div class="header-elements d-none">
        <div class="d-flex justify-content-center">
          <a href="{{ route('admin.trackerMileages.create') }}" class="btn btn-primary">
            <span>Add Tracker Mileage <i class="icon-plus3 ml-2"></i></span>
          </a>
        </div>
      </div>
    </div>
        ...
        <table class="table datatable-colvis-basic dataTable">
          <thead>
            <tr>
              <th>Serial no</th>
              <th>Vehicle No</th>
              <th>Before Peak 02 Hour</th>
              <th>KMS Driven Peak</th>
              <th>After Peak 01 Hour</th>
              <th class="text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            @foreach($trackerMileages as $key => $value)
              <tr>
                <td>{{$value->serial_no}}</td>
                <td>{{$value->vehicle->vehicle_no}}</td>
                <td>{{$value->before_peak_two_hour}}</td>
                <td>{{$value->kms_driven_peak}}</td>
                <td>{{$value->after_peak_one_hour}}</td>
                  ...
                  <div class="list-icons">
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        <a href="{{ route('admin.trackerMileages.show', $value->id) }}" class="dropdown-item">
                          <i class="icon-eye"></i> View Details
                        </a>
                        <a href="{{ route('admin.trackerMileages.edit', $value->id) }}" class="dropdown-item">
                          <i class="icon-pencil7"></i> Edit
                        </a>
                        <form action="{{ route('admin.trackerMileages.destroy', $value->id) }}" method="POST">
                          @csrf
                          @method('DELETE')

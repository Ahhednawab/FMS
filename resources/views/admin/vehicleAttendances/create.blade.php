...
@section('content')
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-lg-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Vehicle Attendance
                        Management</span></h4>
            </div>
            <div class="header-elements d-none">
                <div class="d-flex justify-content-center">
                    <a href="{{ route('vehicleAttendances.index') }}" class="btn btn-primary">
    ...
    <div class="content">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('vehicleAttendances.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-1">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="ibcCenter"
                                            value="{{ $value['ibcCenter'] }}" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select
                                            class="custom-select attendanceStatus @error('status.' . $value['vehicle_id']) is-invalid @enderror"
                                            name="status[{{ $value['vehicle_id'] }}]"
                                ...
                                <div id="additionalOptions-{{ $value['vehicle_id'] }}" class="col-md-2 additionalOptions"
                                    style="display: none;">
                                    <div class="form-group">
                                        <strong>Pool Vechicle</strong>
                                        <select class="custom-select select2 pool_id"
                                            name="pool_id[{{ $value['vehicle_id'] }}]" id="pool_id">
                                            <option value="">ALL</option>
                                            @foreach ($poolvehicles as $station)
                                                <option value="{{ $station->id }}"
                                                    {{ old('pool_id') == $station->id ? 'selected' : '' }}>
                                                    {{ $station->vehicle_no }}
                                                </option>
                                            @endforeach
                                        </select>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            function refreshAllPoolDropdowns() {
                $('.pool_id').each(function() {
                    ...
                    const vehicleId = $select.closest('[id^="additionalOptions-"]').attr('id').split('-')[
                    1];
                    ...
                    $options.each(function() {
                        ...
                        if (selectedPoolVehicles.includes(poolVehicleId) && poolVehicleId !==
                            currentValue) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('[id^="attendanceStatus-"]').forEach(function(attendanceStatusSelect) {
                const vehicleId = attendanceStatusSelect.id.split('-')[
                1]; // Extract vehicle ID from the dropdown ID
                const additionalOptionsDiv = document.getElementById('additionalOptions-' + vehicleId);

                // Function to toggle the additional options based on the selected status
                function toggleAdditionalOptions() {
                    const selectedValue = attendanceStatusSelect
                    .value; // Get the value of the selected option

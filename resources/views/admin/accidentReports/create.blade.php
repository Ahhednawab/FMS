...
@section('title', 'Add Accident Reports')

@section('content')
  <div class="page-header page-header-light">
    <div class="page-header-content header-elements-lg-inline">
      <div class="page-title d-flex">
          <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Accident Reports Management</span></h4>
      </div>
      <div class="header-elements d-none">
        <div class="d-flex justify-content-center">
          <a href="{{ route('admin.accidentReports.index') }}" class="btn btn-primary">
            <span>View Accident Reports <i class="icon-list ml-2"></i></span>
          </a>
        </div>
      </div>
    </div>
  ...
  <div class="content">
    <div class="card">
      <div class="card-body">
        <form action="{{ route('admin.accidentReports.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
          ...
          <div class="row">
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <input value="{{$accident_report_id}}" name="accident_report_id" type="text" class="form-control" readonly>
              </div>
            </div>
              ...
              <div class="form-group">
                ...
                <select class="custom-select" name="witness_involved">
                  ...
                  @foreach($status as $key => $value)
                    <option value="{{$key}}" {{ old('witness_involved') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
              ...
              <div class="form-group">
                ...
                <select class="custom-select" name="primary_cause">
                  ...
                  @foreach($primary_cause as $key => $value)
                    <option value="{{$key}}" {{ old('primary_cause') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select class="custom-select" name="medical_provided">
                  <option value="">--Select--</option>
                  @foreach($status as $key => $value)
                    <option value="{{$key}}" {{ old('medical_provided') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select class="custom-select" name="police_report_filed">
                  <option value="">--Select--</option>
                  @foreach($status as $key => $value)
                    <option value="{{$key}}" {{ old('police_report_filed') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
              ...
              <div class="form-group">
                ...
                <select class="custom-select" name="investigation_status">
                  ...
                  @foreach($investigation_status as $key => $value)
                    <option value="{{$key}}" {{ old('investigation_status') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select class="custom-select" name="insurance_claimed">
                  <option value="">--Select--</option>
                  @foreach($status as $key => $value)
                    <option value="{{$key}}" {{ old('insurance_claimed') == $key ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>

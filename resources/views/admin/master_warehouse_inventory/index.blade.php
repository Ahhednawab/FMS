...
<div class="container-fluid">
    ...
    <a href="{{ route('admin.master_warehouse_inventory.create') }}" class="btn btn-primary mb-3">
        Add New Inventory
    </a>
                            ...
                            <td>
                                @if($item->quantity > 0)
                                    <form class="assign-form d-flex gap-2" data-inventory-id="{{ $item->id }}">
                                        @csrf
                                        <select name="qty" class="form-select form-select-sm" style="width:90px" required>
...
document.querySelectorAll('.assign-form').forEach(form => {
    form.addEventListener('submit', async function(e) {
       ...
       if (!qty || !warehouseId) {
        ...
        try {
            const response = await fetch('{{ route("admin.master_warehouse_inventory.assign") }}', {
                method: 'POST',
                headers: {
